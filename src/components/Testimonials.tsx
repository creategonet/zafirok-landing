...
export function Testimonials() {
  return (
    <section id="testimoniale" className="relative scroll-mt-28 py-20 sm:py-28">
      <div className="mx-auto max-w-6xl px-5 sm:px-6">
        ...
        <div className="mt-12 grid gap-4 sm:mt-16 sm:gap-6 lg:grid-cols-3">
          {testimonials.map((t, i) => (
            <motion.figure
              ...
              className="flex flex-col rounded-3xl border border-line bg-surface/50 p-6 backdrop-blur-sm sm:p-8 transition-colors duration-300 hover:border-sapphire-400/30"
            >
              ...
              <blockquote className="mt-5 flex-1 text-[0.9375rem] leading-relaxed text-slate-300 sm:text-base">
                „{t.quote}”
              </blockquote>
              <figcaption className="mt-7 flex items-center gap-4 border-t border-line pt-6">
                <span
                  className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-full text-sm font-bold ${t.accent}`}
                >
                  {t.initials}
                </span>
                <span>
